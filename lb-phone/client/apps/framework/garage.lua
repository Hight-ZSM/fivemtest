...
local function BringCar(data, cb)
    ...
    lib.TriggerCallback("phone:garage:valetVehicle", function(vehicleData,zhuangtai)
        if zhuangtai then
            TriggerEvent("phone:sendNotification", {
                app = "Garage",
                title = "代客泊车",
                content = "由于出租车在线 代客泊车停止",
            })
            return 
        end
...
--[[  local vehicleLabel = GetDisplayNameFromVehicleModel(model):lower()
    ...
    --return vehicleLabel
  --  function GetVehicleBrandByHash(hash)
        for k, v in pairs(QBCore.Shared.Vehicles) do
            local vehiclemodel = GetEntityModel(v.model)
            if model == vehiclemodel then
                return QBCore.Shared.Vehicles[k].brand .. " " .. QBCore.Shared.Vehicles[k].name
            end
        end
  --  end
         return model
